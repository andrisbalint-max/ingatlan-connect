...
export async function runMarketMonitorForOrg(
  ...
  for (const row of recentRows ?? []) {
    const raw = (row.key_data as { sources?: unknown } | null)?.sources;
    // Régebbi riportsorok `key_data`-ja más szerkezetű lehet, ezért csak
    // tömböt fogadunk el — különben a ciklus futásidőben elszállna.
    const sources: StoredSource[] = Array.isArray(raw) ? (raw as StoredSource[]) : [];
  ...
  try {
    const result = await generateTextWithWebSearch({
      ...
      organizationId,
      maxTokens: 2500,
      // Hat keresési kifejezés van a listában (ld. QUERIES).
      maxSearches: 6,
    });
  ...
  if (existing) {
    const previousRaw = (existing.key_data as { sources?: unknown } | null)?.sources;
    const merged: StoredSource[] = Array.isArray(previousRaw)
      ? [...(previousRaw as StoredSource[])]
      : [];
