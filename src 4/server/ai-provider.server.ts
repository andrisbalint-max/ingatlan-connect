...
export interface GenerateTextInput {
  ...
  temperature?: number;
  /**
   * Hány web-keresést végezhet EGY hívás (csak a kereséses változatnál).
   * KÖLTSÉG: keresésenként kb. 0,01 USD, ezért érdemes a tényleges
   * keresési listához igazítani. Alapérték: 6.
   */
  maxSearches?: number;
}
...
/**
 * Biztonságos tömbbé alakítás a szolgáltatói válaszokhoz.
 *
 * FONTOS: az Anthropic web-keresés eredményblokkja (`web_search_tool_result`)
 * a `content` mezőben vagy találatok TÖMBJÉT adja vissza, vagy — hiba esetén —
 * egyetlen OBJEKTUMOT (`{type:"web_search_tool_result_error", error_code:...}`,
 * pl. `max_uses_exceeded`, `unavailable`). A korábbi kód mindkettőt tömbként
 * járta be, ezért ilyenkor "object is not iterable" hibával elszállt.
 * Ugyanez igaz az OpenAI Responses válaszának beágyazott listáira is.
 */
function asArray<T>(value: unknown): T[] {
  return Array.isArray(value) ? (value as T[]) : [];
}

/** Az Anthropic web-keresés hibakódja, ha a blokk hibát ad vissza. */
function webSearchErrorCode(value: unknown): string | null {
  if (Array.isArray(value) || typeof value !== "object" || value === null) return null;
  const record = value as { type?: string; error_code?: string };
  if (record.type === "web_search_tool_result_error") {
    return record.error_code ?? "unknown";
  }
  return null;
}
...
export async function generateTextWithWebSearch(
  ...
  const maxSearches = input.maxSearches ?? 6;
  ...
  if (provider === "openai") {
    ...
    for (const item of asArray<{
      ...
      for (const block of asArray<{ text?: string; annotations?: unknown }>(item.content)) {
        if (typeof block.text === "string") parts.push(block.text);
        for (const annotation of asArray<{ url?: string }>(block.annotations)) {
      ...
      tools: [
        {
          ...
          name: "web_search",
          // A korlát fölött a szolgáltató `max_uses_exceeded` hibablokkot ad
          // vissza (ezt már lekezeljük), de érdemes a keresési listához
          // igazítani, különben a lista vége el sem indul.
  ...
  for (const block of asArray<{
    ...
    for (const citation of asArray<{ url?: string }>(block.citations)) {
      if (citation.url) sources.add(citation.url);
    }
    if (block.type === "web_search_tool_result") {
      const errorCode = webSearchErrorCode(block.content);
      if (errorCode) {
        // Nem dobunk hibát: a modell a már megtalált találatokból is tud
        // válaszolni. Csak naplózzuk, hogy látszódjon a keresés korlátja.
        searchErrors.push(errorCode);
        continue;
      }
      for (const result of asArray<{ url?: string }>(block.content)) {
  ...
  if (searchErrors.length > 0) {
    console.warn(
      `[ai-provider] Anthropic web search reported errors: ${searchErrors.join(", ")} ` +
        `(organization ${organizationId}); continuing with ${sources.size} source(s).`,
    );
  }
