...
const STATUS_LABELS: Record<string, string> = {
  javasolt: "Javasolt",
  jovahagyva: "emailQueue.status.jovahagyva" as MessageKey,
  elvetve: "Elvetve",
};
...
export function ProjectCompaniesSection({
  ...
  const approve = useMutation({
    ...
    onSuccess: ({ hunter }) => {
      ...
      toast.success(
        hunter > 0
          ? t("projectCompanies.approvedWithHunter", { count: hunter })
          : t("projectCompanies.approved"),
      );
    },
  ...
  const addManual = useMutation({
    mutationFn: async () => {
      if (!manualCompanyId) throw new Error(t("projectCompanies.chooseCompany"));
    ...
    onSuccess: () => {
      ...
      setManualOpen(false);
      setManualCompanyId("");
      toast.success(t("projectCompanies.toast.linked"));
    },
    onError: (error: Error) => toast.error(error.message),
  });
  ...
  const outreach = useMutation({
    ...
    onSuccess: (result) => {
      if (result.status === "ok") {
        toast.success(t("projectCompanies.toast.draft"));
      } else {
        toast.info(result.message ?? t("projectCompanies.toast.noEmail"));
      }
    },
    onError: (error: Error) => toast.error(error.message),
  });
  ...
  return (
    <section className="space-y-3">
              ...
              <li key={link.id} className="rounded-xl border border-border bg-card p-4 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <p className="text-sm font-medium text-foreground">
                    {link.companies?.name ?? t("overview.responses.unknownCompany")}
                  </p>
                  <span
                ...
                {duplicate && (
                  <p className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-medium text-amber-800">
                    ...
                    {t("projectCompanies.alsoIn", { title: duplicate.title })}
                  </p>
                )}
      ...
      <Dialog open={manualOpen} onOpenChange={setManualOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("projectCompanies.addManual")}</DialogTitle>
          </DialogHeader>
          <div className="space-y-2">
            <Label>{t("projectCompanies.fromCrm")}</Label>
            <Select value={manualCompanyId} onValueChange={setManualCompanyId}>
              <SelectTrigger>
                <SelectValue placeholder={t("projectCompanies.chooseCompany")} />
              </SelectTrigger>
              <SelectContent>
                {selectable.length === 0 ? (
                  <SelectItem value="none" disabled>
                    {t("projectCompanies.noCompany")}
                  </SelectItem>
                ) : (
