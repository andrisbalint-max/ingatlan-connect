...
export function ProjectOptenMatchesSection({ projectId }: { projectId: string }) {
  ...
  const askAi = useMutation({
    ...
    onSuccess: (result) => {
      if (result.status === "ok") {
        ...
        if (picked.length === 0) setNotice(t("optenMatch.aiNoCategory"));
      } else {
        setNotice(result.message ?? t("optenMatch.aiFailed"));
      }
    },
  ...
  const saveCategories = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t("optenMatch.toast.categoriesSaved"));
    },
  ...
  const findMatches = useMutation({
    mutationFn: async () => {
      ...
      if (targets.length === 0) throw new Error(t("optenMatch.error.saveFirst"));
    ...
    onSuccess: ({ created }) => {
      ...
      toast.success(
        created > 0
          ? t("optenMatch.toast.newSuggestions", { count: created })
          : t("optenMatch.toast.noNew"),
      );
    },
    onError: (error: Error) => toast.error(error.message),
  });
  ...
  const addManual = useMutation({
    ...
    onSuccess: () => {
      setManualTerm("");
      queryClient.invalidateQueries({ queryKey: ["project-opten-matches", projectId] });
      toast.success(t("optenMatch.toast.added"));
    },
    onError: (error: Error) => toast.error(error.message),
  });
  ...
  const approve = useMutation({
    mutationFn: async (match: MatchRow) => {
      ...
      if (!prospect) throw new Error(t("optenMatch.error.noData"));
    ...
    onSuccess: ({ savedContacts }) => {
      ...
      toast.success(
        savedContacts > 0
          ? t("optenMatch.approvedWithContacts", { count: savedContacts })
          : t("optenMatch.approvedToCrm"),
      );
    },
    onError: (error: Error) => toast.error(error.message),
  });
  ...
  return (
    <section className="space-y-5">
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-foreground">{t("optenMatch.categoriesTitle")}</h3>

        {categories.length === 0 ? (
          <p className="rounded-lg border border-border bg-secondary/40 px-3 py-2 text-sm text-muted-foreground">
            {t("optenMatch.categoriesEmpty")}
          </p>
        ) : (
          <>
      ...
      <div className="space-y-2">
        <h3 className="text-sm font-semibold text-foreground">{t("projectCompanies.addManual")}</h3>
        <Input
          value={manualTerm}
          onChange={(e) => setManualTerm(e.target.value)}
          placeholder={t("optenMatch.searchPlaceholder")}
          aria-label={t("optenMatch.searchLabel")}
        />
      ...
      <div className="space-y-4">
        <h3 className="text-sm font-semibold text-foreground">{t("projectCompanies.title")}</h3>
        {matchesLoading ? (
                        ...
                        <div className="min-w-0 flex-1 space-y-1">
                          ...
                          {prospect?.decision_maker_email ? (
                            <p className="flex flex-wrap items-center gap-2 text-xs text-foreground">
                              {t("optenMatch.decisionMaker", { name: prospect.decision_maker_name })}
