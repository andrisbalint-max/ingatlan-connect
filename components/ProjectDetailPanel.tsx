...
const STATUS_OPTIONS = [
  { value: "aktiv", label: "project.status.aktiv" as MessageKey },
  { value: "eloketszites", label: "project.status.eloketszites" as MessageKey },
  { value: "szunetel", label: "project.status.szunetel" as MessageKey },
  { value: "lezarva", label: "project.status.lezarva" as MessageKey },
];
...
export function ProjectDetailPanel({
  ...
  const deleteFile = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t("project.files.toast.deleted"));
    },
    onError: (error: Error) => toast.error(error.message),
  });
    ...
    try {
      for (const file of Array.from(list)) {
        ...
        try {
          ...
          if (result.status === "ok") {
            toast.success(t("project.files.toast.uploaded", { name: file.name }));
          } else if (result.status === "no_provider") {
            setAiNotice(t("ai.noProvider"));
          } else if (result.status === "out_of_credit") {
            setAiNotice(t("ai.outOfCreditShort"));
          } else {
            toast.info(result.message ?? t("project.ai.failed"));
          }
        } catch (err) {
          toast.info(err instanceof Error ? err.message : t("project.ai.failed"));
        }
        queryClient.invalidateQueries({ queryKey: ["project-files", project.id] });
  ...
  async function download(file: FileRow) {
    ...
    if (error || !data?.signedUrl) {
      toast.error(error?.message ?? t("project.files.downloadFailed"));
      return;
    }
            ...
            <div className="mt-6 space-y-6">
              ...
              <section className="space-y-4">
                ...
                {suggestion && (
                  <div className="space-y-3 rounded-xl border border-primary/30 bg-accent p-4">
                    <p className="flex items-center gap-1.5 text-xs font-medium text-primary">
                      <Sparkles className="size-3.5" strokeWidth={1.5} /> {t("project.ai.hint")}
                    </p>
                    {suggestion.description && (
                      <div className="space-y-1">
                        <Label className="text-xs">{t("project.ai.suggestedDescription")}</Label>
                        <Textarea
                          rows={3}
                    ...
                    {suggestion.targetAudience && (
                      <div className="space-y-1">
                        <Label className="text-xs">{t("project.ai.suggestedAudience")}</Label>
                        <Textarea
                          rows={2}
                ...
                <div className="space-y-2">
                  <Label htmlFor="project-description">{t("project.field.description")}</Label>
                  <Textarea
                ...
                <div className="space-y-2">
                  <Label htmlFor="project-audience">{t("project.field.audience")}</Label>
                  <Textarea
                    id="project-audience"
                    rows={3}
                    value={targetAudience}
                    onChange={(e) => setTargetAudience(e.target.value)}
                    placeholder={t("project.field.audiencePlaceholderLong")}
                  />
                </div>

                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="project-city">{t("crm.field.city")}</Label>
                    <Input
                      id="project-city"
                      value={city}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="project-size">{t("project.field.size")}</Label>
                    <Input
                      id="project-size"
                      inputMode="decimal"
                      value={sizeSqm}
                ...
                <div className="space-y-2">
                  <Label>{t("crm.filter.status")}</Label>
              ...
              <section className="space-y-3">
                ...
                {filesLoading ? (
                  ...
                  <p className="text-sm text-muted-foreground">{t("project.files.empty")}</p>
                ) : (
                        ...
                        {file.ai_summary && (
                          <div className="mb-3 rounded-lg bg-accent p-3">
                            <p className="mb-1 flex items-center gap-1.5 text-xs font-medium text-primary">
                              <Sparkles className="size-3.5" strokeWidth={1.5} /> {t("project.files.aiSummary")}
                            </p>
