...
const STATUS_OPTIONS: Array<{ value: string; label: MessageKey }> = [
  { value: "aktiv", label: "project.status.aktiv" },
  { value: "eloketszites", label: "project.status.eloketszites" },
  { value: "szunetel", label: "project.status.szunetel" },
  { value: "lezarva", label: "project.status.lezarva" },
];
...
function Projects() {
  ...
  const createProject = useMutation({
    mutationFn: async () => {
      ...
      if (!form.title.trim()) throw new Error(t("project.error.titleRequired"));
          ...
          {projects.map((project) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                >
                  {(() => { const o = STATUS_OPTIONS.find((s) => s.value === project.status); return o ? t(o.label) : project.status; })()}
                </span>
              </div>
              {project.description && (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="description">{t("project.field.description")}</Label>
              <Textarea
                id="description"
                rows={3}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="city">{t("crm.field.city")}</Label>
                <Input
                  id="city"
                  value={form.city}
              ...
              <div className="space-y-2">
                <Label htmlFor="size">{t("project.field.size")}</Label>
                <Input
                  id="size"
                  inputMode="decimal"
                  value={form.size_sqm}
            ...
            <div className="space-y-2">
              <Label htmlFor="audience">{t("project.field.audienceOptional")}</Label>
              <Textarea
                id="audience"
                rows={2}
                value={form.target_audience}
                onChange={(e) => setForm({ ...form, target_audience: e.target.value })}
                placeholder={t("project.field.audiencePlaceholder")}
              />
            </div>
            <div className="space-y-2">
